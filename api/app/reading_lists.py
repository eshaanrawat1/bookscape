from __future__ import annotations

import json
from pathlib import Path


class ReadingListStore:
    def __init__(self, root: Path) -> None:
        self.path = root / "user_data" / "reading_lists.json"
        self.path.parent.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            self._write({"lists": []})

    def _read(self) -> dict:
        with self.path.open("r", encoding="utf-8") as f:
            return json.load(f)

    def _write(self, payload: dict) -> None:
        with self.path.open("w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)

    @staticmethod
    def _normalize(name: str) -> str:
        return " ".join((name or "").strip().split())

    def list_all(self) -> list[dict]:
        payload = self._read()
        return payload.get("lists", [])

    def create_list(self, name: str) -> dict:
        clean = self._normalize(name)
        if not clean:
            raise ValueError("List name is required")
        payload = self._read()
        lists = payload.setdefault("lists", [])
        if any(x.get("name", "").lower() == clean.lower() for x in lists):
            raise ValueError("List already exists")
        row = {"name": clean, "books": []}
        lists.append(row)
        self._write(payload)
        return row

    def delete_list(self, name: str) -> None:
        clean = self._normalize(name)
        payload = self._read()
        lists = payload.setdefault("lists", [])
        next_lists = [x for x in lists if x.get("name", "").lower() != clean.lower()]
        if len(next_lists) == len(lists):
            raise KeyError("List not found")
        payload["lists"] = next_lists
        self._write(payload)

    def rename_list(self, name: str, new_name: str) -> dict:
        clean = self._normalize(name)
        clean_new = self._normalize(new_name)
        if not clean_new:
            raise ValueError("List name is required")
        payload = self._read()
        lists = payload.setdefault("lists", [])
        if any(x.get("name", "").lower() == clean_new.lower() and x.get("name", "").lower() != clean.lower() for x in lists):
            raise ValueError("List already exists")
        for row in lists:
            if row.get("name", "").lower() == clean.lower():
                row["name"] = clean_new
                self._write(payload)
                return row
        raise KeyError("List not found")

    def add_book(self, name: str, book_id: str) -> dict:
        clean = self._normalize(name)
        if not book_id:
            raise ValueError("book_id is required")
        payload = self._read()
        lists = payload.setdefault("lists", [])
        for row in lists:
            if row.get("name", "").lower() != clean.lower():
                continue
            books = row.setdefault("books", [])
            if book_id not in books:
                books.append(book_id)
                self._write(payload)
            return row
        raise KeyError("List not found")

    def remove_book(self, name: str, book_id: str) -> dict:
        clean = self._normalize(name)
        payload = self._read()
        lists = payload.setdefault("lists", [])
        for row in lists:
            if row.get("name", "").lower() != clean.lower():
                continue
            books = row.setdefault("books", [])
            row["books"] = [x for x in books if x != book_id]
            self._write(payload)
            return row
        raise KeyError("List not found")


class LikedBooksStore:
    def __init__(self, root: Path) -> None:
        self.path = root / "user_data" / "liked_books.json"
        self.path.parent.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            self._write({"book_ids": []})

    def _read(self) -> dict:
        with self.path.open("r", encoding="utf-8") as f:
            return json.load(f)

    def _write(self, payload: dict) -> None:
        with self.path.open("w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)

    def list_all(self) -> list[str]:
        payload = self._read()
        out = payload.get("book_ids", [])
        if not isinstance(out, list):
            return []
        return [str(x) for x in out if x]

    def add(self, book_id: str) -> None:
        if not book_id:
            raise ValueError("book_id is required")
        payload = self._read()
        ids = payload.setdefault("book_ids", [])
        if book_id not in ids:
            ids.append(book_id)
            self._write(payload)

    def remove(self, book_id: str) -> None:
        payload = self._read()
        ids = payload.setdefault("book_ids", [])
        payload["book_ids"] = [x for x in ids if x != book_id]
        self._write(payload)


class ReadingProgressStore:
    def __init__(self, root: Path) -> None:
        self.path = root / "user_data" / "reading_progress.json"
        self.path.parent.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            self._write({"entries": {}})

    def _read(self) -> dict:
        with self.path.open("r", encoding="utf-8") as f:
            return json.load(f)

    def _write(self, payload: dict) -> None:
        with self.path.open("w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)

    def list_all(self) -> dict:
        payload = self._read()
        entries = payload.get("entries", {})
        if not isinstance(entries, dict):
            return {}
        return {str(k): v for k, v in entries.items() if k}

    def upsert(self, book_id: str, entry: dict) -> dict:
        if not book_id:
            raise ValueError("book_id is required")
        payload = self._read()
        entries = payload.setdefault("entries", {})
        entries[book_id] = entry
        self._write(payload)
        return entry
